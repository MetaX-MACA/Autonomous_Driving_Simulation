...
from agents.llm_utils import use_llm_query, llm_query
...
def filter_left_lane(input_map,v=8):
    # thres_min = 5
    thres_min = -1 # 扩大左转时的候选点范围
    thres_max = 30
    output = {}
    centerline = input_map['centerline']
    boundary = input_map['boundary']
    # right_index = (centerline[:,1] > thres_min) & (centerline[:,1] < thres_max) & ((centerline[:,3]-centerline[:,1]) >= 0)
    right_index = (centerline[:,1] > thres_min) & (centerline[:,1] < thres_max) & (centerline[:,0] > 25) # 扩大左转时的候选点范围
    filtered_centerline = centerline[right_index,:]
...
def rot_and_trans_point(input_points,current_pose):
    ...
    points = transform_points_directly(points,ego_vec,current_vec)
    return points
...
def transform_node_to_lane(input_map,pre_transform=True):
    """
    将输入地图的边界和中心线point数据转换为一种更适合表示车道的形式
    """
    output_lane_map = {}
    edge_lanes = []
    for edge in input_map['boundary']:
        N = edge.shape[0]
        edge_lane = np.zeros((N,6)) # [point, next_point, 0, 0] 最后一个0好像没用上？
        edge_lane[:,:2] = edge[:,:2]
    ...
    for i, centerline in enumerate(input_map['centerline']):
        ...
        if N > 0:
            centerline_lane = np.zeros((N,6)) # [point, next_point, 1, close/away]
