...
from llm_utils import use_llm_query, llm_query
...
class MotionAgent:
    ...
    def func_placement_and_motion_single_vehicle(self, scene, added_car_name):
        ...
        all_current_vertices_coord = scene.all_current_vertices_coord # 当前地图中的所有点坐标
        # 添加所有车辆第一帧的起始点坐标
        ...
        if one_added_car['need_placement_and_motion'] is True: 
            ...
            # 若是逆行，将transformed_map_data的中心线数据进行反转和调整，使车辆的运动方向与正常方向相反
            if one_added_car['wrong_way'] == 'true':
            ...
            try:
                motion_result = vehicle_motion(
                    ...
                    origin_extrinsics=scene.current_extrinsics
                )
            ...
            # 检测多个动态车辆之间的碰撞，并调整车辆的运动轨迹以避免相互碰撞。它在多个车辆的轨迹上检测碰撞，并基于优先级判断，调整其中一辆车的轨迹
