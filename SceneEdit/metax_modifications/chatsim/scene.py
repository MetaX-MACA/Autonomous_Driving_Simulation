...
class Scene(nn.Module):
    def __init__(self, config):
        ...
        self.init_img_dir = os.path.join(self.data_root, self.scene_name, config['init_img_dir']) # 由其它方式渲染得到的图片
        ...
        self.current_extrinsics = self.nerf_motion_extrinsics[::3] # 间隔3个，读取原视频每帧的居中视角的外参。背景图采取预先生成的方式得到
        self.is_ego_motion = True
    ...
    def setup_cars(self):
        ...
        # 使用其他场景的 bbox_file 文件时，需注释这部分代码。不然可能因为图像分辨率不同，导致box越界
